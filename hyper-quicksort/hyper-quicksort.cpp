...
* N (number of values) should be multiple of p (nr. of processes)
* p (nr. of processes) should be a power of 2
*/
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
int main(int argc, char **argv)
{
    ...
    if (world_rank == 0)
    {
        ...
        if (file)
        {
            fscanf(file, "%d", &n);             // number of values
            A = (int *)malloc(sizeof(int) * n); // array of n values (ints)
            for (int i = 0; i < n; i++)
                fscanf(file, "%d", &A[i]); // read values from file
        }
        fclose(file);
    }
    ...
    {
        //printf("iter : %d\n",iter);
        // create communicator based on nr. of iteration
